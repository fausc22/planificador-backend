// server-integrado.js - Servidor integrado con sistema de planificación
...
// Importar sistema de base de datos existente
...
// Importar rutas del planificador
const planificadorRoutes = require('./routes/planificador-routes');

// Importar middlewares del planificador
const errorHandler = require('./middlewares/errorHandler');
const AppError = require('./utils/AppError');

// Importar worker de notificaciones del planificador
...
const allowedOrigins = [
    ...
    // Dominio de producción - Tienda y Panel
    ...
    // Frontend del planificador en Vercel
    'https://planificador-ps.vercel.app',
    'https://planificador-ps.vercel.app/',
...
morgan.token('response-time-colored', (req, res) => {
    ...
    if (responseTime > 1000) return `\x1b[31m${responseTime}ms\x1b[0m`;
    if (responseTime > 500) return `\x1b[33m${responseTime}ms\x1b[0m`;
    return `\x1b[32m${responseTime}ms\x1b[0m`;
});
...
// Express automáticamente ignora multipart/form-data (lo maneja multer)
...
// ==============================================
// MIDDLEWARE PARA TIMEOUTS DE PROXY (nginx)
// ==============================================
// Agregar headers para aumentar timeout del proxy
app.use((req, res, next) => {
    // Aumentar timeout para subida de archivos
    if (req.path.includes('/registrar-con-foto') || req.path.includes('/upload')) {
        res.setTimeout(120000); // 2 minutos
        // Headers para nginx/reverse proxy
...
const RATE_LIMITS = {
    ...
    // Planificador - límites moderados
    planificador: {
        max: 2000,
        routes: ['/api/planificador']
    }
};
...
// ARCHIVOS ESTÁTICOS DEL PLANIFICADOR (fotos de empleados y logueos)
app.use('/uploads', express.static('public/uploads', staticOptions));
app.use('/api/uploads', express.static('public/uploads', staticOptions));
...
logApp('✅ Rutas de API existentes configuradas', 'success', 'ROUTES');

// ==============================================
// RUTAS DEL PLANIFICADOR
// ==============================================
app.use('/planificador', planificadorRoutes);
logApp('✅ Rutas del planificador configuradas', 'success', 'ROUTES');
...
app.get('/health', async (req, res) => {
    try {
        ...
        const healthData = {
            ...
            endpoints: {
                tienda: '/store, /admin, /estadisticas',
...
// Usar el error handler del planificador también (para errores específicos)
...
const startServer = async () => {
    try {
        logApp('🚀 Iniciando servidor integrado (Tienda + Planificador)...', 'info', 'STARTUP');
        
        // Inicializar base de datos existente
        await initializeDatabase();
        
        // Crear directorios necesarios para el planificador
        const planificadorDirs = [
            'public/uploads/empleados',
            'public/uploads/logueos',
            'auth',
            'logs'
        ];
        
        planificadorDirs.forEach(dir => {
            const fullPath = path.join(__dirname, dir);
            if (!fs.existsSync(fullPath)) {
                fs.mkdirSync(fullPath, { recursive: true });
                logApp(`📁 Directorio creado: ${dir}`, 'info', 'STARTUP');
            }
        });
        
        // Iniciar worker de notificaciones del planificador (si está configurado)
        if (process.env.ADMIN_PHONE) {
            logApp('📱 Iniciando worker de notificaciones del planificador...', 'info', 'STARTUP');
            notificacionesWorker.iniciarWorker();
        } else {
            logApp('⚠️  ADMIN_PHONE no configurado, notificaciones deshabilitadas', 'warn', 'STARTUP');
        }
        
        // Solo servidor HTTP (puerto 4000)
        const server = app.listen(port, '0.0.0.0', () => {
            logApp(`🌟 ¡Servidor integrado iniciado exitosamente!`, 'success', 'STARTUP');
            ...
            logApp(`📋 Planificador: Sistema de Planificación de Empleados`, 'info', 'STARTUP');
            ...
            logApp(`   - Planificador Frontend: ${process.env.PLANIFICADOR_FRONTEND_URL || 'No configurado'}`, 'info', 'CONFIG');
        });
        
        // Configurar timeout del servidor (aumentado para subida de fotos)
        server.timeout = 120000; // 120 segundos (2 minutos) para permitir subida de fotos grandes
        server.keepAliveTimeout = 125000; // Mayor que timeout
        server.headersTimeout = 126000; // Mayor que keepAliveTimeout
